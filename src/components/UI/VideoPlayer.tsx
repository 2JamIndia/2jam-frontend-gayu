...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
	...
	React.useEffect(() => {
		if (currentIndexLevels) {
			VideoRef.current.getInternalPlayer("hls").currentLevel = currentIndexLevels;
		}
	}, [
		currentIndexLevels
	]);
			...
			<div className={`video-player-content ${play ? "play" : ""}`}>
				{Hls.isSupported() ? (
					<>
						...
						{stateSelectQuality ? (
							<div
								className={`video-player-content-quality-select ${stateSelectQualityAnimationClose
									? "close"
									: ""
									}`}
